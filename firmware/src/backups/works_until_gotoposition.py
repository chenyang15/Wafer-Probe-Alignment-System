...
class ControlPanel(QWidget):
    ...
    def _run(self, fn):
        ...
        Same pattern as jog. All motion methods use this."""
        def _worker():
            fn()
            QTimer.singleShot(0, self.main_window.refresh_position)
    ...
    def _goto(self):
        ...
        def do_goto():
            if not self.pico.homed:
                self.pico.home("ALL")
            # Y coordinate is inverted: positive Y in UI = downward = negative in motor space
            self.pico.goto(x, -y)
        self._run(do_goto)
...
class ConfigPanel(QWidget):
    def __init__(self):
        ...
        self._spin_row(layout,"Length (mm)",25.0,1,300,1)
